...
interface TeamLeaderboardHeaderProps {
    sortBy: TeamSortKey;
    onSortChange: (key: TeamSortKey) => void;
}

export function TeamLeaderboardHeader({ sortBy, onSortChange }: TeamLeaderboardHeaderProps) {
    return (
        <div className="hidden lg:grid grid-cols-12 gap-2 px-4 py-3 border-b border-gray-700/50 text-gray-400 text-xs font-bold uppercase tracking-wider">
            ...
            <button
                ...
                className={`col-span-2 text-center hover:text-white transition-colors ${sortBy === 'total' ? 'text-white' : ''}`}
            >
                Total {sortBy === 'total' && '▼'}
            </button>
        </div>
